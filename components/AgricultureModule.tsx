...
const AgricultureModule: React.FC = () => {
  ...
  const analyzePlant = async () => {
    ...
    try {
      ...
      // Step 2: Confidence Evaluation & Decision Logic
      if (hfData.fallback === false && hfData.result) {
        // High confidence result from local model (> 75%)
        setHealthResult(hfData.result);
      } else {
        // Low confidence (<= 75%) or error - trigger Step 3: Gemini Fallback
        const prompt = `
      ...
      console.error("Local inference failed, falling back to Gemini:", error);
      // Fallback to existing Gemini implementation if backend is unavailable or fails
      try {
        const prompt = `
          Analyze this plant image. 
          Identify the plant species. 
            ...
            <div className="border-l border-slate-100 pl-0 md:pl-12 flex flex-col justify-center">
              {irrigationAdvice ? (
                <div className="animate-fade-in">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-2xl border border-blue-100 shadow-inner">
                    <div 
                      className="text-slate-700 leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: parseMarkdownText(irrigationAdvice) }}
                    />
                  </div>
                </div>
              ) : (
