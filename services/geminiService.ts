...
if (!apiKey) {
  console.error("CRITICAL ERROR: API_KEY is missing in the environment. Please add 'API_KEY' to your Vercel/Netlify Environment Variables settings.");
}
...
export const getCityAirQuality = async (city: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3.1-flash-lite',
      contents: `Find the real-time Air Quality Index (AQI) for ${city}. 
...
export const getAirQualityByCoordinates = async (lat: number, lon: number): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3.1-flash-lite',
      contents: `Find the real-time Air Quality Index (AQI) for the location at Latitude ${lat}, Longitude ${lon}. Identify the nearest city/area.
      Provide the response in this structure:
